...
- **16-step sequencer** with per-track color-coded on/off grid
...
- **Mute / Solo / Volume** per track with visual feedback (muted rows dim, solo rows glow)
- **Keyboard shortcuts**: `Space` play/stop, `M` mute, `S` solo, `D` delete selected step
...
├── js/
│   ├── sequencer.js    # Step scheduler, BPM/swing, play/pause, mute/solo
